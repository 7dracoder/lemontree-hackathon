    ...
    <div id="donor-dashboard" className="p-8 space-y-8 max-w-7xl mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-card border border-border p-5">
          ...
          <p className="text-[11px] tracking-wide uppercase text-secondary mb-5">{'// '}Most subscribed resources</p>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={topBySubscriptions} layout="vertical">
              ...
              <YAxis type="category" dataKey="name" tick={{ fill: '#71717A', fontSize: 10, fontFamily: 'JetBrains Mono' }} width={120} axisLine={false} tickLine={false} />
      ...
      <div className="bg-card border border-border p-5">
        ...
        <p className="text-[11px] tracking-wide uppercase text-secondary mb-5">{'// '}Count of resources by average rating</p>
        <ResponsiveContainer width="100%" height={160}>
          <BarChart data={ratingBuckets}>
            <CartesianGrid strokeDasharray="2 2" stroke="#27272A" vertical={false} />
            <XAxis dataKey="range" tick={{ fill: '#71717A', fontSize: 10, fontFamily: 'JetBrains Mono' }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fill: '#71717A', fontSize: 10, fontFamily: 'JetBrains Mono' }} axisLine={false} tickLine={false} />
            <Tooltip contentStyle={{ background: '#0F0F10', border: '1px solid #FACC15', borderRadius: 0, fontFamily: 'JetBrains Mono', fontSize: 12, textTransform: 'uppercase' }} cursor={{ fill: '#27272A', opacity: 0.4 }} />
            <Bar dataKey="count" fill="#22C55E" radius={[0, 0, 0, 0]} barSize={40} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="bg-card border border-border">
        <div className="p-4 border-b border-border flex items-center justify-between bg-page">
          ...
          <span className="text-[10px] tracking-widest font-bold uppercase text-accent bg-accent/10 px-2 py-1 border border-accent/20">
            TOP 100 LIST
          </span>
        </div>
            ...
            <tbody className="divide-y divide-border">
              {[...data].sort((a, b) => (b._count?.resourceSubscriptions ?? 0) - (a._count?.resourceSubscriptions ?? 0)).slice(0, 100).map(r => (
                <tr key={r.id} className="table-row hover:bg-surface transition-colors">
                  ...
                  <td className="px-4 py-3 text-secondary uppercase tracking-wide">{r.city ?? '—'}, {r.state ?? ''}</td>
                  <td className="px-4 py-3 text-tertiary uppercase tracking-wide text-[10px]">
                    {lang === 'es' ? (r.resourceType?.name_es ?? r.resourceType?.name) : r.resourceType?.name}
                  </td>
                  <td className="px-4 py-3 text-status-success font-bold">{r._count?.resourceSubscriptions ?? 0}</td>
                  <td className="px-4 py-3 text-status-info font-bold">{r._count?.reviews ?? 0}</td>
